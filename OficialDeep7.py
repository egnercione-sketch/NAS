# ============================================
# NBA ANALYTICS SUITE v1.6 (Enhanced Modular Version with Boost Mode)
# ============================================
...
TEAM_STATS_AVG = {
    "PTS": 112.5, "REB": 44.2, "AST": 24.8, "PRA": 181.5,
    "STL": 7.2, "BLK": 5.1, "3PM": 12.3
}

TRIXIE_WEIGHT_MAP = {
    "main": {"basePRA": 1.0, "MinutesSafe": 0.8, "LowVariance": 0.6, "UsageSpike": 0.2, 
             "corrPenalty": 1.0, "underdog": 0.2, "high_pace": 0.1},
    "upside": {"basePRA": 0.8, "UsageSpike": 1.2, "BenchMonster": 1.3, 
               "GarbageTimeHistorico": 0.9, "corrPenalty": 0.6, "underdog": 0.3, "high_pace": 0.2},
    ...
    "boost": {"basePRA": 0.7, "CeilingPRA": 1.5, "UsageSpike": 1.4, 
              "StealPotential": 1.1, "BlockPotential": 1.1, "ThreePointPotential": 1.2,
...
# ============================================================================
# IMPORTS DOS NOVOS MÓDULOS (FASE 1)
# ============================================================================
...
FEATURE_FLAGS = {
    ...
    "DYNAMIC_THRESHOLDS": True,      # NOVO
    "CONTEXTUAL_SCORING": True,      # NOVO
    "BOOST_MODE": True               # NOVO
}
...
def calculate_dynamic_threshold(stat, player_data=None, team_data=None):
    ...
    if player_data:
        ...
        pos_multiplier = {
            "PG": {"PTS": 1.2, "AST": 1.5, "REB": 0.7},
            "SG": {"PTS": 1.3, "AST": 1.1, "REB": 0.8},
            "SF": {"PTS": 1.1, "AST": 0.9, "REB": 1.2},
            "PF": {"PTS": 1.0, "AST": 0.8, "REB": 1.5},
            "C": {"PTS": 0.9, "AST": 0.6, "REB": 1.8}
        }
        
        if position in pos_multiplier and stat in pos_multiplier[position]:
            threshold *= pos_multiplier[position][stat]
    
    return max(threshold * 0.8, threshold)
...
def build_trixies_with_enhancements(team_players_ctx, game_ctx, trixie_type="main"):
    ...
    if trixie_type == "boost" and config.get("boost_mode", True):
        ...
        # Se enhanced_system estiver disponível, usar
        ...
        if enhanced_system and ENHANCED_SYSTEM_AVAILABLE:
            ...
            # Geração normal baseada no tipo
            if trixie_type == "main":
                trixies = build_trixies_for_game_main(team_players_ctx, game_ctx)
            elif trixie_type == "upside":
                trixies = build_trixies_for_game_upside(team_players_ctx, game_ctx)
    ...
    # Aplicar correlation filters se disponível
...
# ============================================================================
# DvP MODULE (de dvp_module.py) - CORRIGIDO
# ============================================================================
...
# ============================================================================
# FUNÇÕES AUXILIARES PARA TRIXIES
# ============================================================================
...
def escolher_mercado_para_jogador(p, tags, trixie_type="main"):
    """
    Escolhe o mercado para um jogador baseado nas tags e tipo de trixie.
    
    Args:
        p: Dicionário do jogador
        tags: Lista de tags do jogador
        trixie_type: Tipo de trixie ("main", "upside", "highvalue", "boost")
    
    Returns:
        Dicionário com mercado escolhido
    """
    if not tags:
        return None
    
    # Diferentes prioridades para cada tipo de trixie
    if trixie_type == "boost":
        # Boost Mode: Foco em tetos explosivos
        prioridade = [
            ("CeilingExplosion", "PRA"),
            ("StealPotential", "STL"),
            ("BlockPotential", "BLK"),
            ("ThreePointPotential", "3PM"),
            ("UsageSpike", "PRA"),
            ("PivoSobrevivente", "REB"),
            ("BenchMonster", "PRA"),
            ("GarbageTimeHistorico", "PTS"),
            ("DVPPointsMatchup", "PTS"),
            ("DVPReboundMatchup", "REB"),
            ("DVPAssistMatchup", "AST")
        ]
    elif trixie_type == "highvalue":
        # HighValue: REB > AST > PTS > 3PM > BLK > STL
        prioridade = [
            ("DVPReboundMatchup", "REB"),
            ("DVPAssistMatchup", "AST"), 
            ("DVPPointsMatchup", "PTS"),
            ("BigRebound", "REB"),
            ("PivoSobrevivente", "REB"),
            ("HiddenReboundValue", "REB"),
            ("ReboundMatchup", "REB"),
            ("HiddenAssistValue", "AST"),
            ("AssistMatchup", "AST"),
            ("SynergyRebAst", "AST"),
            ("TripleThreat", "PRA"),
            ("HybridRole", "PRA"),
        ...
        # Upside: Foco em potencial de explosão
        prioridade = [
            ("UsageSpike", "PRA"),
            ("HiddenReboundValue", "REB"),
            ("HiddenAssistValue", "AST"),
    ...
    for tese, mercado in prioridade:
        if tese in tags:
            best_tese = tese
            best_mercado = mercado
            break
    
    if best_tese is None:
        return None
    
    # Calcular valor base baseado no mercado
    if best_mercado == "REB":
        base = p.get("reb_L5", 0)
        stat_name = "reb_L5"
    elif best_mercado == "AST":
        base = p.get("ast_L5", 0)
        stat_name = "ast_L5"
    elif best_mercado == "PTS":
        base = p.get("pts_L5", 0)
        stat_name = "pts_L5"
    elif best_mercado == "STL":
        base = p.get("stl_L5", 0) if "stl_L5" in p else 0
        stat_name = "stl_L5"
    elif best_mercado == "BLK":
        base = p.get("blk_L5", 0) if "blk_L5" in p else 0
        stat_name = "blk_L5"
    elif best_mercado == "3PM":
        base = p.get("3pm_L5", 0) if "3pm_L5" in p else 0
        stat_name = "3pm_L5"
    else:  # PRA
        base = p.get("pra_L5", 0)
        stat_name = "pra_L5"
    
    # Aplicar ajuste DvP se disponível
    dvp_data = p.get("dvp_data", {})
    if dvp_data:
        multiplier = dvp_data.get("overall", 1.0)
        base = base * multiplier
    
    # Aplicar ajuste de pace se disponível
    if p.get("pace_adjusted", False):
        pace_factor = p.get("pace_factor", 1.0)
        base = base * pace_factor
    
    return {
        "tipo": best_mercado,
        "base_valor": round(base, 1),
        "tese_usada": best_tese,
        "stat_name": stat_name
    }

# ============================================================================
# NOVO: FUNÇÕES DE DISPLAY ATUALIZADAS (FASE 5.1)
# ============================================================================

def display_trixie_simple(trixie, rank, trixie_type="main"):
    """Versão atualizada com mais detalhes e teses"""
    players = trixie.get("players", [])
    score = trixie.get("score", 0)
    enhanced = trixie.get("enhanced", False)
    
    st.markdown('<div class="trixie-card">', unsafe_allow_html=True)
    
    # Cabeçalho com ícone do tipo
    type_icons = {
        "main": "🎯",
        "upside": "🚀", 
        "highvalue": "💰",
    ...
    icon = type_icons.get(trixie_type, "🏆")
    
    if enhanced:
        st.markdown(f'<div class="trixie-title">{icon} TRIXIE {trixie_type.upper()} AVANÇADA #{rank} — Score: {score:.2f}</div>', unsafe_allow_html=True)
        st.caption("✅ Sistema Avançado aplicado")
    else:
        st.markdown(f'<div class="trixie-title">{icon} TRIXIE {trixie_type.upper()} #{rank} — Score: {score:.2f}</div>', unsafe_allow_html=True)
    ...
    with st.expander("📊 Detalhes do Score"):
        ...
        for p in players:
            ...
            # Stats principais
            stats_text = f"• MIN: {p.get('min_L5', 0):.1f} | PTS: {p.get('pts_L5', 0):.1f} | "
            stats_text += f"REB: {p.get('reb_L5', 0):.1f} | AST: {p.get('ast_L5', 0):.1f} | "
            stats_text += f"PRA: {p.get('pra_L5', 0):.1f}"
            st.write(stats_text)
            
            # Ceilings se disponível
            if "proj_ceil_95_pra" in p:
                st.write(f"• Ceiling PRA: {p.get('proj_ceil_95_pra', 0):.1f} (p95)")
            
            # Teses ativas (máximo 3 principais)
            tags = p.get("tags", [])
            if tags:
                main_tags = tags[:3]
                st.write(f"• Teses: {', '.join(main_tags)}")
                if len(tags) > 3:
                    st.write(f"  + {len(tags)-3} teses adicionais")
            
            # Matchup DvP
            dvp_data = p.get("dvp_data", {})
            if dvp_data and dvp_data.get("overall", 1.0) != 1.0:
                st.write(f"• DvP Multiplier: {dvp_data.get('overall', 1.0):.3f}")
    
    # Jogadores em colunas
    for i, p in enumerate(players):
        col1, col2 = st.columns([3, 2])
        
        with col1:
            st.markdown(f"**{p.get('team')} - {p.get('name')}**")
            
            # Posição e experiência
            pos_exp = f"{p.get('position')} | "
            if p.get('exp'):
                pos_exp += f"EXP: {p.get('exp')} anos | "
            pos_exp += f"{p.get('role')}"
            st.caption(pos_exp)
            
            # Stats principais compactos
            stats_text = f"PRA: {p.get('pra_L5', 0):.1f} | PTS: {p.get('pts_L5', 0):.1f} | "
            stats_text += f"REB: {p.get('reb_L5', 0):.1f} | AST: {p.get('ast_L5', 0):.1f}"
            st.write(stats_text)
            
            # Potenciais específicos para Boost Mode
            if trixie_type == "boost":
                boost_stats = []
                if p.get("stl_L5", 0) >= 1.5:
                    boost_stats.append(f"STL: {p.get('stl_L5', 0):.1f}")
                if p.get("blk_L5", 0) >= 1.2:
                    boost_stats.append(f"BLK: {p.get('blk_L5', 0):.1f}")
                if p.get("3pm_L5", 0) >= 2.0:
                    boost_stats.append(f"3PM: {p.get('3pm_L5', 0):.1f}")
                
                if boost_stats:
                    st.caption(f"🔥 Potenciais: {' | '.join(boost_stats)}")
        
        with col2:
            mercado = p.get("mercado", {})
            if mercado:
                # Mostrar a tese específica que gerou o mercado
                tese_usada = mercado.get('tese_usada', 'N/A')
                help_text = f"Tese: {tese_usada}"
                ...
                # Guardar tese usada para auditoria
                p["tese_efetiva"] = tese_usada
            
            # Contexto do jogador
            context_items = []
            if p.get("is_underdog"):
                context_items.append("UNDERDOG")
            if p.get("team_injuries", 0) > 0:
                context_items.append(f"{p['team_injuries']} lesões")
            if p.get("matchup_score", 1.0) > 1.1:
                context_items.append("DvP +")
            
            if context_items:
                st.caption(f"🎯 {' | '.join(context_items)}")
    
    # Botão para auditoria
    if st.session_state.get("audit_system"):
        if st.button(f"📋 Registrar para Auditoria", key=f"audit_{trixie_type}_{rank}"):
            audit_system = st.session_state.audit_system
            # ... código para registrar na auditoria ...
            st.success("Registrado na auditoria!")
...
def score_player_for_boost_mode(p, game_ctx):
    ...
    # Ritmo alto do jogo
    if game_ctx.get("pace_expected", 0) > 102:
        base += 8.0
    
    # Underdog em jogo de ritmo alto
    if p.get("is_underdog", False) and game_ctx.get("pace_expected", 0) > 102:
        base += 6.0
...
# ============================================================================
# FUNÇÕES DE SCORING ATUALIZADAS (FASE 4)
# ============================================================================
...
def build_trixies_for_game_boost(team_players_ctx, game_ctx):
    """Gera trixies para Boost Mode (foco em tetos explosivos)"""
    # Filtrar jogadores com potencial de explosão
    candidates = []
    for _, plist in team_players_ctx.items():
        for p in plist:
            # Critérios para Boost Mode
            tags = p.get("tags", [])
            ...
            if "proj_ceil_95_pra" in p:
                ceiling_pra = p.get("proj_ceil_95_pra", 0)
                baseline_pra = p.get("pra_L5", 0)
                has_high_ceiling = ceiling_pra > baseline_pra * 1.3
            
            if has_potential or has_high_ceiling:
                if p.get("expected_minutes", 0) >= 18:  # Mínimo de minutos
                    candidates.append(p)
    ...
    scored.sort(key=lambda x: x[0], reverse=True)
    top_players = [p for s, p in scored[:10]]  # Top 10 para mais combinações
    ...
    for trio in combinations(top_players, 3):
        ...
        # Garantir diversidade de posições (pelo menos 1 guard e 1 big)
...
def show_audit_page():
    """Página de auditoria atualizada"""
    ...
    st.subheader("📈 Efetividade por Tese")
    
    # Calcular hit rate por tese
    if st.button("📊 Calcular Efetividade das Teses"):
        ...
        for entry in audit_system.audit_data:
            if "validation" in entry and entry["validation"].get("overall") == "GREEN":
                for player in entry.get("players", []):
                    tese_usada = player.get("tese_efetiva")
                    if tese_usada:
                        if tese_usada not in tese_stats:
                            tese_stats[tese_usada] = {"hits": 0, "total": 0}
                        tese_stats[tese_usada]["hits"] += 1
                    # Contar total
                    for tese in player.get("tags", []):
                        if tese not in tese_stats:
                            tese_stats[tese] = {"hits": 0, "total": 0}
                        tese_stats[tese]["total"] += 1
        ...
        if tese_stats:
            df_tese = pd.DataFrame([
                ...
                for tese, stats in tese_stats.items()
            ])
            st.dataframe(df_tese.sort_values("Hit Rate", ascending=False), use_container_width=True)
    ...
    with col2:
        trixie_types = ["main", "upside", "highvalue", "boost", "todos"]
        trixie_type_filter = st.selectbox("Tipo de Trixie", trixie_types)
    ...
    for idx, entry in enumerate(audit_data):
        with st.expander(f"📝 Trixie {idx+1}: {entry.get('game', {}).get('away', '?')} @ {entry.get('game', {}).get('home', '?')} - {entry.get('trixie_type', 'N/A').upper()} - Score: {entry.get('score', 0):.2f}"):
...
# ============================================================================
# UI PRINCIPAL COM MENU LATERAL COMPLETO (ATUALIZADO) - FASE 5.2
# ============================================================================
...
def main():
    ...
    st.markdown(f"**Data:** {TODAY} • **Versão:** Sistema Modular Completo + Auditoria + Boost Mode")
    
    # Carregar dados iniciais
    safe_load_initial_data()
    
    # Menu lateral COMPLETO com todas as páginas (ATUALIZADO - FASE 5.2)
    ...
    menu_options = [
        ...
        "🚀 TRIXIES (Upside)",
        "💰 TRIXIES (High Value)",
        "⚡ TRIXIES (Boost Mode)",  # NOVO - FASE 5.2
        "🧪 Sistema Avançado",
        "📋 Auditoria",
        "⚙️ Config",
        "🧪 Testes de Validação"  # NOVO - FASE 8.1
    ]
    ...
    if choice == "🏠 Dashboard":
        ...
        # NOVA SEÇÃO: Status do Sistema Avançado
        st.subheader("🧪 Sistema Avançado v1.6")
        
        col_adv1, col_adv2, col_adv3, col_adv4 = st.columns(4)
        
        with col_adv1:
            status = "✅ ATIVO" if st.session_state.get("use_advanced_features", False) else "❌ INATIVO"
            st.metric("Status Sistema", status)
        
        with col_adv2:
            if st.session_state.get("use_advanced_features", False):
                config = st.session_state.get("advanced_features_config", {})
                active = sum(config.values())
                st.metric("Módulos Ativos", f"{active}/6")
            else:
                st.metric("Módulos Ativos", "0/6")
        
        with col_adv3:
            boost_active = st.session_state.get("advanced_features_config", {}).get("boost_mode", False)
            st.metric("Boost Mode", "✅" if boost_active else "❌")
        
        with col_adv4:
            if st.button("⚙️ Configurar Sistema"):
                # Redirecionar para página do sistema avançado
                st.session_state.menu_choice = "🧪 Sistema Avançado"
                st.rerun()
            ...
            if not games:
                ...
                for idx,g in enumerate(games):
                    ...
                    try:
                        ...
                        norm_away = [normalize_roster_entry(x) for x in list_away]
                        norm_home = [normalize_roster_entry(x) for x in list_home]
                        
                        im = st.session_state.injuries_monitor
                        def mark_out_flags(norm_list, team_abbr):
                            for r in norm_list:
                                pname = r.get("PLAYER")
                                flag = ""
                                if _status_is_out_or_questionable(r.get("STATUS")): flag="OUT"
                                if im and hasattr(im, "is_player_out"):
                                    try:
                                        if im.is_player_out(pname, team_abbr): flag="OUT"
                                    except Exception: pass
                                r["DEPT_MEDICO"] = flag
                            return norm_list
                        
                        norm_away = mark_out_flags(norm_away, away)
                        norm_home = mark_out_flags(norm_home, home)
                        
                        c1,c2 = st.columns(2)
                        with c1:
                            st.markdown(f"**{away}** ({len(norm_away)} jogadores)")
                            if norm_away:
                                st.dataframe(pd.DataFrame(norm_away), use_container_width=True)
                            else:
                                st.warning(f"Nenhum jogador encontrado para {away}")
                        
                        with c2:
                            st.markdown(f"**{home}** ({len(norm_home)} jogadores)")
                            if norm_home:
                                st.dataframe(pd.DataFrame(norm_home), use_container_width=True)
                            else:
                                st.warning(f"Nenhum jogador encontrado para {home}")
    ...
    # ============================================================================
    # TRIXIES (MAIN) - ATUALIZADO
    # ============================================================================
    elif choice == "🎯 TRIXIES (Main)":
        st.header("🎯 TRIXIES (Principais)")
        
        # VALIDAÇÃO CRÍTICA
        critical_ok, checks = validate_pipeline_integrity(['l5', 'scoreboard'])
        if not critical_ok:
            st.error("❌ Dados insuficientes para gerar trixies. Por favor, atualize os dados na aba Config.")
            st.stop()
        ...
        if not games:
            ...
            indices = ["Todos"] + [f"{i}: {g.get('away')} @ {g.get('home')}" for i,g in enumerate(games)]
            sel = st.selectbox("Selecione jogo para gerar Trixies", indices, index=0)
            
            # Verificar se estamos usando sistema avançado
            use_advanced = st.session_state.get("use_advanced_features", False)
            if use_advanced:
                st.info("🧪 **SISTEMA AVANÇADO ATIVADO**")
                config = st.session_state.get("advanced_features_config", {})
                active_modules = []
                if config.get("dynamic_thresholds"): active_modules.append("Dynamic Thresholds")
                if config.get("contextual_scoring"): active_modules.append("Contextual Scoring")
                if config.get("pace_adjuster"): active_modules.append("Pace Adjuster")
                if config.get("vacuum_matrix"): active_modules.append("Vacuum Matrix")
                if config.get("correlation_filters"): active_modules.append("Correlation Filters")
                if active_modules:
                    st.success(f"Módulos ativos: {', '.join(active_modules)}")
            
            if st.button("Gerar Trixies (Principais)"):
                df_l5 = st.session_state.df_l5
                if df_l5 is None or (hasattr(df_l5,"empty") and df_l5.empty):
                    st.info("Cache L5 vazio. Coletando L5...")
                    df_l5 = get_players_l5(progress_ui=True)
                    st.session_state.df_l5 = df_l5
                
                l5_index, l5_index_norm = build_l5_indices(df_l5)
                overrides = st.session_state.name_overrides or {}
                im = st.session_state.injuries_monitor
                odds = st.session_state.odds or {}
                ...
                for idx,g in enumerate(games):
                    ...
                    script_type, script_notes = classify_game_script(game_ctx)
                    game_ctx["script_type"] = script_type
                    game_ctx["script_notes"] = script_notes
                    
                    roster_away = load_json(os.path.join(CACHE_DIR, f"roster_{away}.json")) or fetch_team_roster(away, progress_ui=False)
                    roster_home = load_json(os.path.join(CACHE_DIR, f"roster_{home}.json")) or fetch_team_roster(home, progress_ui=False)
                    
                    list_away = extract_list(roster_away); list_home = extract_list(roster_home)
                    rows_away = [normalize_roster_entry(a) for a in list_away]
                    rows_home = [normalize_roster_entry(a) for a in list_home]
                    
                    rows_away = [r for r in rows_away if not _status_is_out_or_questionable(r.get("STATUS"))]
                    rows_home = [r for r in rows_home if not _status_is_out_or_questionable(r.get("STATUS"))]
                    
                    if im and hasattr(im, "is_player_out"):
                        try:
                            rows_away = [r for r in rows_away if not im.is_player_out(r["PLAYER"], away)]
                            rows_home = [r for r in rows_home if not im.is_player_out(r["PLAYER"], home)]
                        except Exception: pass
                    
                    ctx_away={"team_abbr": away, "team_injuries": 0, "spread": float(game_ctx.get("spread") or 0.0),
                              "is_underdog": (float(game_ctx.get("spread") or 0.0)>0), "is_b2b": False,
                              "pace_expected": game_ctx.get("pace_expected")}
                    ctx_home={"team_abbr": home, "team_injuries": 0, "spread": -float(game_ctx.get("spread") or 0.0),
                              "is_underdog": (float(game_ctx.get("spread") or 0.0)<0), "is_b2b": False,
                              "pace_expected": game_ctx.get("pace_expected")}
                    opp_ctx_away={"opponent_reb_rank":0,"opponent_ast_rank":0, "opponent_team": home}
                    opp_ctx_home={"opponent_reb_rank":0,"opponent_ast_rank":0, "opponent_team": away}
                    
                    def build_team_players_ctx(rows, team_ctx, opp_ctx, opp_roster):
                        players_ctx=[]
                        for r in rows:
                            name=r["PLAYER"]
                            l5row,_ = resolve_l5_row(name, l5_index, l5_index_norm, overrides)
                            
                            # USAR VERSÃO APRIMORADA SE ATIVADO
                            if use_advanced:
                                pctx = build_player_ctx_with_enhancements(
                                    r, l5row, team_ctx, opp_ctx, game_ctx, dvp_analyzer
                                )
                            else:
                                pctx = build_player_ctx(r, l5row, team_ctx, opp_ctx, dvp_analyzer)
                            
                            tags = gerar_tags_para_jogador(pctx, game_ctx, rows, opp_roster)
                            pctx["tags"]=tags
                            mercado = escolher_mercado_para_jogador_tipo(pctx, tags, "main")
                            pctx["mercado"]=mercado
                            players_ctx.append(pctx)
                        return players_ctx
                    
                    team_players_ctx = {
                        "AWAY": build_team_players_ctx(rows_away, ctx_away, opp_ctx_away, rows_home),
                        "HOME": build_team_players_ctx(rows_home, ctx_home, opp_ctx_home, rows_away),
                    }
                    
                    # USAR FUNÇÃO ATUALIZADA
                    trixies = build_trixies_with_enhancements(team_players_ctx, game_ctx, "main")
                    
                    # Registrar na auditoria
                    audit_system = st.session_state.get("audit_system")
                    if audit_system and trixies:
                        for trixie in trixies[:5]:  # Registrar apenas top 5
                            game_info = {
                                "away": away,
                                "home": home,
                                "gameId": g.get("gameId"),
                            ...
                            audit_system.log_trixie(trixie, game_info, "main")
                    
                    st.subheader(f"{away} @ {home} — Script: {game_ctx.get('script_type')} — {game_ctx.get('script_notes')}")
                    
                    if use_advanced:
                        st.write(f"🧪 **Trixies geradas com Sistema Avançado:** {len(trixies)}")
                        if any(t.get("enhanced", False) for t in trixies):
                            st.success("✅ Ajustes de pace e filtros aplicados!")
                    else:
                        st.write(f"Trixies geradas: {len(trixies)} (máx {PERFORMANCE_CONFIG['max_trixies_per_game']['main']})")
                    
                    if trixies:
                        for rank, trixie in enumerate(trixies, start=1):
                            display_trixie_simple(trixie, rank, "main")
                    else:
                        st.write("Nenhuma TRIXIE forte para este jogo.")
                    st.markdown("---")
    
    # ============================================================================
    # TRIXIES (UPSIDE) - ATUALIZADO
    # ============================================================================
    elif choice == "🚀 TRIXIES (Upside)":
        st.header("🚀 TRIXIES (Upside)")
        ...
        if not games:
            ...
            indices = ["Todos"] + [f"{i}: {g.get('away')} @ {g.get('home')}" for i,g in enumerate(games)]
            sel = st.selectbox("Selecione jogo para Upside", indices, index=0)
            
            if st.button("Gerar Trixies (Upside)"):
                df_l5 = st.session_state.df_l5
                if df_l5 is None or (hasattr(df_l5,"empty") and df_l5.empty):
                    st.info("Cache L5 vazio. Coletando L5...")
                    df_l5 = get_players_l5(progress_ui=True)
                ...
                for idx,g in enumerate(games):
                    ...
                    def build_team_players_ctx(rows, team_ctx, opp_ctx, opp_roster):
                        ...
                        for r in rows:
                            ...
                            tags = gerar_tags_para_jogador(pctx, game_ctx, rows, opp_roster)
                            pctx["tags"]=tags
                            mercado = escolher_mercado_para_jogador_tipo(pctx, tags, "upside")
                            pctx["mercado"]=mercado
                            players_ctx.append(pctx)
                        return players_ctx
                    ...
                    # USAR FUNÇÃO ATUALIZADA
                    trixies_up = build_trixies_with_enhancements(team_players_ctx, game_ctx, "upside")
                    
                    # Registrar na auditoria
                    audit_system = st.session_state.get("audit_system")
                    if audit_system and trixies_up:
                        for trixie in trixies_up[:3]:  # Registrar apenas top 3
                            game_info = {
                                "away": away,
                                "home": home,
                                "gameId": g.get("gameId"),
                            ...
                            audit_system.log_trixie(trixie, game_info, "upside")
                    
                    st.subheader(f"{away} @ {home} — Script: {game_ctx.get('script_type')} — {game_ctx.get('script_notes')}")
                    
                    use_advanced = st.session_state.get("use_advanced_features", False)
                    if use_advanced:
                        st.write(f"🧪 **Trixies Upside com Sistema Avançado:** {len(trixies_up)}")
                    else:
                        st.write(f"Trixies Upside geradas: {len(trixies_up)} (máx {PERFORMANCE_CONFIG['max_trixies_per_game']['upside']})")
                    
                    if trixies_up:
                        for rank, trixie in enumerate(trixies_up, start=1):
                            display_trixie_simple(trixie, rank, "upside")
                    else:
                        st.write("Nenhuma TRIXIE Upside forte para este jogo.")
                    st.markdown("---")
    
    # ============================================================================
    # TRIXIES (HIGH VALUE) - ATUALIZADO
    # ============================================================================
    elif choice == "💰 TRIXIES (High Value)":
        st.header("💰 TRIXIES (High Value)")
        ...
        if not games:
            ...
            indices = ["Todos"] + [f"{i}: {g.get('away')} @ {g.get('home')}" for i,g in enumerate(games)]
            sel = st.selectbox("Selecione jogo para High Value", indices, index=0)
            
            if st.button("Gerar Trixies (High Value)"):
                df_l5 = st.session_state.df_l5
                if df_l5 is None or (hasattr(df_l5,"empty") and df_l5.empty):
                    st.info("Cache L5 vazio. Coletando L5...")
                    df_l5 = get_players_l5(progress_ui=True)
                ...
                for idx,g in enumerate(games):
                    ...
                    def build_team_players_ctx(rows, team_ctx, opp_ctx, opp_roster):
                        ...
                        for r in rows:
                            ...
                            tags = gerar_tags_para_jogador(pctx, game_ctx, rows, opp_roster)
                            pctx["tags"]=tags
                            mercado = escolher_mercado_para_jogador_tipo(pctx, tags, "highvalue")
                            pctx["mercado"]=mercado
                            players_ctx.append(pctx)
                        return players_ctx
                    ...
                    # USAR FUNÇÃO ATUALIZADA
                    trixies_hv = build_trixies_with_enhancements(team_players_ctx, game_ctx, "highvalue")
                    
                    # Registrar na auditoria
                    audit_system = st.session_state.get("audit_system")
                    if audit_system and trixies_hv:
                        for trixie in trixies_hv[:3]:  # Registrar apenas top 3
                            game_info = {
                                "away": away,
                                "home": home,
                                "gameId": g.get("gameId"),
                            ...
                            audit_system.log_trixie(trixie, game_info, "highvalue")
                    
                    st.subheader(f"{away} @ {home} — Script: {game_ctx.get('script_type')} — {game_ctx.get('script_notes')}")
                    
                    use_advanced = st.session_state.get("use_advanced_features", False)
                    if use_advanced:
                        st.write(f"🧪 **Trixies High Value com Sistema Avançado:** {len(trixies_hv)}")
                    else:
                        st.write(f"🎯 **Trixies High Value geradas:** {len(trixies_hv)} (máx {PERFORMANCE_CONFIG['max_trixies_per_game']['highvalue']})")
                    
                    if trixies_hv:
                        for rank, trixie in enumerate(trixies_hv, start=1):
                            display_trixie_simple(trixie, rank, "highvalue")
                    else:
                        st.write("Nenhuma TRIXIE High Value forte para este jogo.")
                    st.markdown("---")
    
    # ============================================================================
    # TRIXIES (BOOST MODE) - NOVO (FASE 5.3)
    # ============================================================================
    elif choice == "⚡ TRIXIES (Boost Mode)":
        st.header("⚡ TRIXIES (Boost Mode)")
        
        # VALIDAÇÃO CRÍTICA
        critical_ok, checks = validate_pipeline_integrity(['l5', 'scoreboard'])
        if not critical_ok:
            st.error("❌ Dados insuficientes para gerar trixies. Atualize os dados na aba Config.")
            st.stop()
        ...
        if not games:
            ...
            indices = ["Todos"] + [f"{i}: {g.get('away')} @ {g.get('home')}" for i,g in enumerate(games)]
            sel = st.selectbox("Selecione jogo para Boost Mode", indices, index=0)
            
            st.info("""
            **⚡ Boost Mode - Foco em Tetos Explosivos**
            
            Este modo prioriza jogadores com alto potencial (ceiling) para explosões estatísticas.
            Ideal para lineups agressivas que buscam retornos acima da média.
            
            **Características:**
            • Foco em ceilings (p95) do ProjectionEngine
            • Potenciais específicos (steals, blocks, 3PM)
            • Matchups DvP muito favoráveis
            • Contexto de jogo com alto ritmo
            """)
            
            # Verificar se Boost Mode está ativo
            if not st.session_state.get("advanced_features_config", {}).get("boost_mode", True):
                st.warning("⚠️ Boost Mode está desativado nas configurações avançadas.")
                if st.button("Ativar Boost Mode"):
                    st.session_state.advanced_features_config["boost_mode"] = True
                    st.success("Boost Mode ativado! Recarregue a página.")
                    st.rerun()
            
            if st.button("Gerar Trixies (Boost Mode)", type="primary"):
                df_l5 = st.session_state.df_l5
                if df_l5 is None or (hasattr(df_l5,"empty") and df_l5.empty):
                    st.info("Cache L5 vazio. Coletando L5...")
                    df_l5 = get_players_l5(progress_ui=True)
                ...
                for idx,g in enumerate(games):
                    ...
                    def build_team_players_ctx(rows, team_ctx, opp_ctx, opp_roster):
                        ...
                        for r in rows:
                            ...
                            tags = gerar_tags_para_jogador(pctx, game_ctx, rows, opp_roster)
                            pctx["tags"]=tags
                            mercado = escolher_mercado_para_jogador_tipo(pctx, tags, "boost")
                            pctx["mercado"]=mercado
                            players_ctx.append(pctx)
                        return players_ctx
                    ...
                    # USAR FUNÇÃO ATUALIZADA
                    trixies_boost = build_trixies_with_enhancements(team_players_ctx, game_ctx, "boost")
                    
                    # Registrar na auditoria
                    audit_system = st.session_state.get("audit_system")
                    if audit_system and trixies_boost:
                        for trixie in trixies_boost[:5]:  # Registrar apenas top 5
                            game_info = {
                                "away": away,
                                "home": home,
                                "gameId": g.get("gameId"),
                            ...
                            audit_system.log_trixie(trixie, game_info, "boost")
                    
                    st.subheader(f"{away} @ {home} — Script: {game_ctx.get('script_type')} — {game_ctx.get('script_notes')}")
                    
                    use_advanced = st.session_state.get("use_advanced_features", False)
                    if use_advanced:
                        st.write(f"🧪 **Trixies Boost Mode com Sistema Avançado:** {len(trixies_boost)}")
                        if any(t.get("enhanced", False) for t in trixies_boost):
                            st.success("✅ Ajustes de pace, thresholds dinâmicos e filtros aplicados!")
                    else:
                        st.write(f"⚡ **Trixies Boost Mode geradas:** {len(trixies_boost)} (máx {PERFORMANCE_CONFIG['max_trixies_per_game']['boost']})")
                    
                    if trixies_boost:
                        for rank, trixie in enumerate(trixies_boost, start=1):
                            display_trixie_simple(trixie, rank, "boost")
                    else:
                        st.write("Nenhuma TRIXIE Boost Mode forte para este jogo.")
                    st.markdown("---")
    ...
    # ============================================================================
    # PÁGINA PADRÃO (se nenhuma opção for selecionada)
    # ============================================================================
    else:
        st.header("🏀 Bem-vindo ao NBA Analytics Suite v1.6")
        ...
        with col1:
            if st.button("🏠 Ir para Dashboard"):
                ...
                st.rerun()
        with col2:
            if st.button("🎯 Gerar Trixies"):
                st.session_state.menu_choice = "🎯 TRIXIES (Main)"
                st.rerun()
        with col3:
            if st.button("⚡ Testar Boost Mode"):
                st.session_state.menu_choice = "⚡ TRIXIES (Boost Mode)"
                st.rerun()
