...
def calculate_dynamic_threshold(stat, player_data=None, team_data=None):
    ...
    # aplicar um fator mínimo/máximo
...
def initialize_strategic_engine():
    """Inicializa os componentes do motor estratégico"""
    if "use_strategic_engine" not in st.session_state:
        st.session_state.use_strategic_engine = True  # Ativado por default
    
    # Verificar se módulos estão disponíveis
    if not NOVOS_MODULOS_DISPONIVEIS:
        st.warning("⚠️ Módulos estratégicos não disponíveis. Sistema funcionará em modo de compatibilidade.")
        return
    
    try:
        # Inicializar componentes do motor estratégico
        ...
        logger.info("✅ Motor estratégico inicializado com sucesso")
...
# Gerar teses para todos os jogadores
if "thesis_engine" in st.session_state and st.button("🔍 Gerar Teses Estratégicas"):
...
def display_strategic_trixies():
    """Exibe as trixies estratégicas com verificação de disponibilidade"""
    st.header("🎯 TRIXIE - Motor Estratégico")
    
    # Verificar se o motor estratégico está disponível
    if not st.session_state.get("use_strategic_engine", False) or "strategy_engine" not in st.session_state:
        st.warning("⚡ Motor estratégico não disponível. Por favor, verifique as configurações na aba '🧪 Sistema Avançado'.")
        st.info("O sistema está funcionando em modo de compatibilidade com as funcionalidades originais.")
        return
    ...
    # Gerar recomendações quando o botão é pressionado
    if st.button("🚀 Gerar Recomendações Estratégicas"):
        with st.spinner("Gerando recomendações estratégicas..."):
            try:
                # Inicializar o rotation analyzer para todos os jogos
                rotation_analyzer = st.session_state.get("rotation_analyzer")
                if rotation_analyzer:
                    for game in games:
                        away = game.get("away")
                        home = game.get("home")
                        # Análisar rotações (dados devem estar em st.session_state)
                        pass
...
def main():
    ...
    if choice == "🏠 Dashboard":
        ...
        with col_adv2:
            strategies_available = sum(1 for m in ["thesis_engine", "strategy_engine", "narrative_formatter"] 
                                     if m in st.session_state)
            st.metric("Módulos Ativos", f"{strategies_available}/8")
        with col_adv3:
                ...
                try:
                    ...
                    def process_roster(roster_list, team_abbr, is_home):
                        ...
                        for entry in roster_list:
                            ...
                            expected_minutes = 0  # Será calculado depois
                            role = "deep_bench"
                            if starter:
                                role = "starter"
